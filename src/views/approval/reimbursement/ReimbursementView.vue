...
const fetchDataItem = async (id) => {
  ...
  dataItem.value = [];
  if ("data" in res.data) dataItem.value = res.data.data;
};
...
const approveData = async (payload) => {
  const token = JSON.parse(localStorage.getItem("token"));
  Api.defaults.headers.common.Authorization = `Bearer ${token}`;
  const res = await Api.post(`/approval_reimbursement/approve/${id}`, payload);
  ...
  if (res.data.success) {
    ...
    closeModal();
    router.push({ path: `/approvalreimbursement` });
  } else {
    Swal.fire({
      position: "center",
      icon: "error",
...
const rejectData = async (payload) => {
  if (payload.is_revision == true && !payload.notes) {
    ...
    const token = JSON.parse(localStorage.getItem("token"));
    Api.defaults.headers.common.Authorization = `Bearer ${token}`;
    const res = await Api.post(`/approval_reimbursement/reject/${id}`, payload);
          ...
          <div class="grid grid-cols-2 pl-[71px] gap-y-3 mb-7 pt-7">
            ...
            <div class="flex flex-col gap-2">
              ...
              <input
                ...
                :v-model="dataArr.attachment_path"
                :disabled="true"
              />
              <label
                id="file-input-label"
