...
const filteredItems = (search) => {
  ...
  const filteredR = sortedData.value.filter((item) => {
    (item.uom_name.toLowerCase().indexOf(search.toLowerCase()) > -1) |
      (item.uom_name.toLowerCase().indexOf(search.toLowerCase()) > -1);
    return (
      (item.uom_name.toLowerCase().indexOf(search.toLowerCase()) > -1) |
      (item.uom_name.toLowerCase().indexOf(search.toLowerCase()) > -1)
...
const deleteUom = async (id) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      Api.delete(`/uom/delete_data/${id}`).then((res) => {
        Swal.fire({
          title: "Successfully",
...
const exportToExcel = () => {
  ...
  const worksheet = workbook.addWorksheet("UOM Data");

  const tableHead = [{ title: "Nomor" }, { title: "ID" }, { title: "UOM" }];
  ...
  sortedDataReactive.value.forEach((data, rowIndex) => {
    ...
    worksheet.getCell(rowIndex + 2, 3).value = data.uom_name;
  });
