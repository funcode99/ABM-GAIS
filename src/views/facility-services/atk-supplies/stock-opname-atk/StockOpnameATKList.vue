...
const fetchData = async (
  ...
  const res = await Api.get(
    `/stock_opname/get?page=${page}&id_company=${selectedType}&code_status_doc=${status}&start_date=${start_date}&end_date=${end_date}&search=${search}&perPage=${perpage}`
  );
...
const deleteValue = async (id) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      Api.delete(`/stock_opname/delete_data/${id}`).then((res) => {
        Swal.fire({
          title: "Successfully",
...
const downloadTemplate = async () => {
  const token = JSON.parse(localStorage.getItem("token"));
  Api.defaults.headers.common.Authorization = `Bearer ${token}`;
  Api.get(`/stock_opname/download_template`, {
    responseType: "blob",
  }).then((res) => {
    ...
    link.download = "Template Stock Opname.xlsx";
    link.click();
  });
};
...
const importData = async () => {
  if (fileImport.value) {
    const token = JSON.parse(localStorage.getItem("token"));
    Api.defaults.headers.common.Authorization = `Bearer ${token}`;
    Api.post(`/stock_opname/import_data`, {
