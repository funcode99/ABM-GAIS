...
const fetchData = async (page, selectedType, status, start_date, end_date,search,perpage) => {
  ...
  const res = await Api.get(`/stock_opname/get?page=${page}&id_company=${selectedType}&code_status_doc=${status}&start_date=${start_date}&end_date=${end_date}&search=${search}&perPage=${perpage}`);
