...
import airlinesFormDetail from "@/components/request-trip/modal-step-form/airlines-form-detail.vue";
...
let isVisibleAirlinesFormDetail = ref(false);
...
const inputStylingClass =
  "w-full md:w-52 lg:w-56 py-2 px-4 border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm";

const inputStylingWithoutWidthClass =
  "py-2 px-4 border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm";
const labelStylingClass = "block mb-2 font-JakartaSans font-medium text-sm";
const circleStepBasicStylingClass =
  "rounded-full border border-black w-11 h-11 bg-[#d9d9d9] flex flex-col items-center text-center relative";
...
watch(departureDate, (newValue) => {
  ...
  formattedDepartureDate =
    getDepartureDateYear +
    "/" +
    getDepartureDateMonth +
    "/" +
    getDepartureDateDay;
...
watch(returnDate, (newValue) => {
  ...
  formattedReturnDate =
    getReturnDateYear + "/" + getReturnDateMonth + "/" + getReturnDateDay;
  ...
  if (localStorage.getItem("tripId") !== null) {
    ...
    if (requestType.value[1] == "Site Visit") {
      const api = await Api.post(
        `/request_trip/update_data/${localStorage.getItem("tripId")}`,
        {
          id_zona: zona.value,
...
const submitRequestTrip = async () => {
  ...
  Api.defaults.headers.common.Authorization = `Bearer ${token}`;
  const api = await Api.post(
    `/request_trip/submit/${localStorage.getItem("tripId")}`
  );
  ...
  if (tripId) {
    try {
      ...
      const api = await Api.get(
        `/travel_guest/get_by_travel_id/trip_id/${tripId}`
      );
    ...
    try {
      ...
      Api.defaults.headers.common.Authorization = `Bearer ${token}`;
      const api = await Api.get(
        `/travel_guest/get_by_travel_id/trip_id/${localStorage.getItem(
          "tripId"
        )}`
      );
      travellerGuestTableData.value = api.data.data;
...
const fetchAirlines = async () => {
  try {
    ...
    Api.defaults.headers.common.Authorization = `Bearer ${token}`;
    const api = await Api.get(
      `/flight_trip/get_by_travel_id/trip_id/${localStorage.getItem("tripId")}`
    );
...
const fetchTaxiVoucher = async () => {
  try {
    ...
    Api.defaults.headers.common.Authorization = `Bearer ${token}`;
    const api = await Api.get(
      `/taxi_voucher/get_by_travel_id/trip_id/${localStorage.getItem("tripId")}`
    );
...
const fetchOtherTransportation = async () => {
  try {
    ...
    Api.defaults.headers.common.Authorization = `Bearer ${token}`;
    const api = await Api.get(
      `/other_transport/get_by_travel_id/trip_id/${localStorage.getItem(
        "tripId"
      )}`
    );
...
const fetchAccomodation = async () => {
  try {
    ...
    Api.defaults.headers.common.Authorization = `Bearer ${token}`;
    const api = await Api.get(
      `/accomodation_trip/get_by_travel_id/trip_id/${localStorage.getItem(
        "tripId"
      )}`
    );
...
const fetchCashAdvance = async () => {
  try {
    ...
    Api.defaults.headers.common.Authorization = `Bearer ${token}`;
    const api = await Api.get(
      `/cash_advance/get_by_trip_id/${localStorage.getItem("tripId")}`
    );
...
const addButtonStylingClass =
  "btn btn-success bg-green border-green hover:bg-none capitalize text-white font-JakartaSans text-xs hover:bg-white hover:text-green hover:border-green ml-2";
</script>
...
<template>
  ...
  <Modal
    type="clean"
    v-model:visible="isVisibleOpenModal"
    v-model:offsetTop="modalPaddingHeight"
  >
    <!-- modal title -->
    <modalHeader
      @closeVisibility="isVisibleOpenModal = !isVisibleOpenModal"
    ...
    <div class="flex justify-center pt-3 px-[10px] gap-x-[19px] h-[100px]">
      ...
      <div
        :class="[
          formStep > 0
            ...
            ? 'bg-[#d9d9d9]'
            : 'bg-white',
          circleStepBasicStylingClass,
        ]"
      >
        <h1 class="mt-11 w-11 font-medium text-[10px]">Requester Info</h1>
        <img
          :src="arrow"
          class="absolute top-[14px] bottom-0 right-[-19px] h-5 w-5"
        />
        <img
          :src="check"
          class="absolute top-[12px] bottom-0 h-5 w-5"
          :class="formStep > 0 ? 'block' : 'hidden'"
        />
      </div>
      ...
      >
        <img
          :src="check"
          class="absolute top-[12px] bottom-0 h-5 w-5"
          :class="formStep > 1 ? 'block' : 'hidden'"
        />
        <!-- absolute top-[14px] bottom-0 right-[-19px]  -->
        <h1 class="mt-11 w-11 font-medium text-[10px] text-black fixed">
          Purpose of Trip
        </h1>
      </div>
      ...
      <div
        v-if="requestType[1] == 'Company Business'"
        v-for="data in companyBusinessStepList"
        :class="[
          formStep > data.id
            ? 'bg-[#fff]'
            : formStep === data.id
      ...
      >
        ...
        <img
          :src="arrow"
          class="absolute top-[14px] bottom-0 right-[-19px] h-5 w-5"
          :class="data.id == 7 ? 'hidden' : 'block'"
        />
        <img
          :src="check"
          class="absolute top-[12px] bottom-0 h-5 w-5"
          :class="formStep > data.id ? 'block' : 'hidden'"
        />
      </div>

      <div
        v-if="requestType[1] == 'Site Visit'"
        v-for="data in companyBusinessStepList"
        :class="[
          formStep > data.id
            ? 'bg-[#fff]'
      ...
      <div
        v-if="requestType[1] == 'Field Break'"
        v-for="data in fieldBreakStepList"
        :class="[
          formStep > data.id
            ? 'bg-[#fff]'
            : formStep === data.id
      ...
      <div
        v-if="requestType[1] == 'Taxi Voucher'"
        v-for="data in taxiVoucherStepList"
        :class="[
          formStep > data.id
            ? 'bg-[#fff]'
            : formStep === data.id
      ...
      <div
        v-if="requestType[1] == 'Crewing'"
        v-for="data in crewingStepList"
        :class="[
          formStep > data.id
            ? 'bg-[#fff]'
            : formStep === data.id
    ...
    <button
      v-if="
        formStep == 2 &&
        (requestType[1] == 'Company Business' || requestType[1] == 'Site Visit')
      "
      @click="isVisibleGuest = !isVisibleGuest"
      :class="addButtonStylingClass"
    >
      + Add Guest
    </button>
    ...
    <button
      v-if="
        ...
        (requestType[1] == 'Company Business' || requestType[1] == 'Site Visit')
      "
      @click="isVisibleAirlines = !isVisibleAirlines"
      :class="addButtonStylingClass"
    >
      + Add Airlines
    </button>
    ...
    <button
      v-if="
        ...
        (requestType[1] == 'Company Business' || requestType[1] == 'Site Visit')
      "
      @click="isVisibleTaxiVoucher = !isVisibleTaxiVoucher"
      :class="addButtonStylingClass"
    >
      + Add Taxi Voucher
    </button>
    ...
    <button
      v-if="
        ...
        (requestType[1] == 'Company Business' || requestType[1] == 'Site Visit')
      "
      @click="isVisibleOtherTransportation = !isVisibleOtherTransportation"
      :class="addButtonStylingClass"
    >
    ...
    <button
      v-if="
        ...
        (requestType[1] == 'Company Business' || requestType[1] == 'Site Visit')
      "
      @click="isVisibleAccomodation = !isVisibleAccomodation"
      :class="addButtonStylingClass"
    >
      + Add Accomodation
    </button>
    ...
    <button
      v-if="
        ...
        (requestType[1] == 'Company Business' || requestType[1] == 'Site Visit')
      "
      @click="isVisibleCashAdvance = !isVisibleCashAdvance"
      :class="addButtonStylingClass"
    >
      + Add Cash Advance
    </button>
    ...
    <div class="modal-box-inner-inner mt-4">
      <div :class="formStep == 0 ? 'h-[250px]' : 'max-h-[500px]'">
        ...
        <div class="text-left" :class="formStep == 0 ? 'block' : 'hidden'">
          ...
          <div :class="rowClass">
            ...
            <div :class="columnClass">
              <div class="w-full">
                <span class="block"
                  >Phone <span class="text-red-star">*</span></span
                >
                <input
                  disabled
                  :class="inputStylingClass"
        ...
        <!-- Step 2 FORM -->
        <form
          class="text-left px-4 flex flex-col"
          :class="formStep == 1 ? 'block' : 'hidden'"
          @submit.prevent="savePurposeOfTrip"
        >
          <div :class="columnClass + ' mx-4'">
            ...
            >
              <option
                v-for="data in optionDataPurposeofTrip"
                :value="[data.id, data.document_name]"
              >
                {{ data.document_name }}
              </option>
            </select>
          ...
          >
            ...
            <select
              id="site"
              v-model="siteVisitLocation"
              :class="inputStylingWithoutWidthClass"
              required
            >
              <!-- :required="requestType[1] == 'Site Visit' ? true : false" -->
              <option
                v-for="data in optionDataSite"
                :key="data.id"
                :value="data.id"
              >
                {{ data.site_name }}
          ...
          <div :class="columnClass + ' mx-4 my-3'">
            <label for="attachment"
              >File Attachment<span
                v-if="
                  requestType[1] == 'Site Visit' ||
                  requestType[1] === 'Field Break'
                "
                class="text-[#f5333f]"
                >*</span
              ></label
          ...
          >
            ...
            <label for="da_number"> DA Number </label>
          ...
          <div :class="rowClass">
            ...
            <div :class="columnClass">
              ...
              <select
                class="w-full md:w-52 lg:w-56 py-2 px-4 border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm cursor-pointer mt-2"
                v-model="fromCity"
                required
              >
          ...
          <div :class="rowClass">
            ...
            <!-- Return Date -->
            <div :class="columnClass">
              <span> Return Date<span class="text-red-star">*</span> </span>
              <input
                v-model="returnDate"
                :min="departureDate == '' ? minDate : departureDate"
                type="date"
                :class="inputStylingClass"
                placeholder="Date"
                required
          ...
          <div :class="rowClass" class="hidden">
            ...
            <div class="w-full invisible">
              <div :class="columnClass">
                ...
                >
                  <option
                    v-for="data in optionDataZona"
                    :value="data.zona_name"
                  >
                    {{ data.zona_name }}
                  </option>
                </select>
          ...
          <div class="sticky bottom-0 bg-white" v-if="formStep === 1">
            <div class="flex justify-between font-bold py-2">
              <button
                v-if="formStep > 0"
                @click="formStep--"
                class="border border-blue text-blue py-3 px-11 rounded-lg max-w-[141px]"
              >
                Back
              </button>

              <button
                v-if="formStep < stepCounter"
        ...
        >
          ...
          <div class="px-2" :class="formStep == 4 ? 'block' : 'hidden'">
            <button
              @click="
                isVisibleOtherTransportation = !isVisibleOtherTransportation
              "
              :class="addButtonStylingClass"
            >
              + Add Other Transportation
    ...
    <div
      v-if="formStep !== 1"
      class="fixed left-0 right-0 bg-white bottom-0 px-5 py-2"
    >
      <div class="flex justify-between font-bold">
        ...
        <button
          v-else
          disabled
          class="bg-zinc-300 border border-blue text-white py-3 px-11 rounded-lg max-w-[141px]"
        >
          Back
        </button>
  ...
  <airlinesForm
    :isOpen="isVisibleAirlines"
    @changeVisibility="isVisibleAirlines = false"
    @fetchAirlines="fetchAirlines"
    @submitData="submitAirLinesForm"
  />

  <airlinesFormDetail 
    :isOpen="isVisibleAirlinesFormDetail"
    @changeVisibility="isVisibleAirlinesFormDetail = false"
    @fetchAirlines="fetchAirlines"
    @submitData="submitAirLinesFormDetail"
  />

  <!-- Step 5 Modal Add Taxi Voucher -->
  <taxiVoucherForm
    :isOpen="isVisibleTaxiVoucher"
    @changeVisibility="isVisibleTaxiVoucher = false"
    @fetchTaxiVoucher="fetchTaxiVoucher"
  />
  ...
  <accomodationForm
    :isOpen="isVisibleAccomodation"
    @changeVisibility="isVisibleAccomodation = false"
    @fetchAccomodation="fetchAccomodation"
  />

  <!-- Step 8 Modal Add Cash Advance -->
  <cashAdvanceForm
    :isOpen="isVisibleCashAdvance"
    @changeVisibility="isVisibleCashAdvance = false"
    @fetchCashAdvance="fetchCashAdvance"
  />
</template>
