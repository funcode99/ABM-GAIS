...
<template>
  ...
  <Modal v-model:visible="isVisible" v-model:offsetTop="modalPaddingHeight">
    <main>
      <modalHeader @closeVisibility="isVisible = false" title="New Menu" />
      ...
      <form class="pr-4 modal-box-inner-inner" @submit.prevent="submit">
        ...
        <div class="mb-3 text-left">
          ...
          <select
            id="status_menu"
            :class="inputStylingClass"
            v-model="idStatusMenu"
            required
          >
            <option
              v-for="data in statusMenu"
              :key="data.id"
              :value="data.code"
            >
              {{ data.status }}
        ...
        <div class="mb-3 text-left">
          <label for="sort"> Sort<span class="text-red-star">*</span> </label>
        ...
        <hr class="divide-solid bg-black h-[1px]" />
        ...
        <modalFooter class="mt-6 pt-5" @closeEdit="isVisible = false" />
      </form>
    </main>
  </Modal>
</template>
